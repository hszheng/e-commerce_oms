...
class Page extends React.Component {
    ...
    render() {
        ...
        const columns = [
            ...
            { title: '订单来源', dataIndex: 'replaceBuyerName'},
            { title: '代买金额', dataIndex: 'replaceBuyerPrice', render:(text, item) => {
                let  replaceBuyerPrice = 0;
                _.each(item.products, (item, index) =>{
                    replaceBuyerPrice += item.replaceBuyerPrice; 
                });
                return <span>{item.replaceBuyerPrice ? item.replaceBuyerPrice : replaceBuyerPrice }</span>
            }},
                            ...
                            <Menu.Item>
                                <FormModal2
                                    ...
                                    items={[
                                        ...
                                        { type: 'text', key: 'trackingNo', label: '物流单号', placeholder: '请输入物流单号', required: true },
                                    ]}
        ...
        const expandedRowRender = (record) => (
            <Table className="product-item" rowKey={record => record.name} columns={[
                ...
                // { title: '是否来自代买', width: 100, dataIndex: 'replaceBuyerId',render:(text, item) => (
                //         <span>{ item.replaceBuyerPrice && '√'}</span>
                //     ) },
                // { title: '代买价格', width: 100, dataIndex: 'replaceBuyerPrice', render:(text, item) =>(
                //     <span>{item.replaceBuyerPrice && `￥${item.replaceBuyerPrice.toFixed(2)}`}</span>
                //     )},
                // { title: '操作',width: 100, render: (text, item) => (
                //     <span>
    ...
    _toStatus(status){
        ...
        $('#agentManage .orderEdit .orderStatus div').siblings().removeClass('active');
        if (status === '全部') {
    ...
    _priceConfirm({ nowPrice }, order) {
        ...
        Meteor.call('ReplaceBuyOrder.methods.updatePrice', obj, (err) => {
            if (!err) {
    ...
    _civilNo({ trackingNo, expressCompany }, order) {
        ...
        obj.nowId = order._id;
        obj.no = order.no;
        Meteor.call('ReplaceBuyOrder.methods.updateCivilNo', obj, (err) => {
            if (!err) {
                message.success('修改成功!');
            } else {
                message.error('修改失败!');
    ...
    _confirmShip({ transferNo, transferCompany }, order) {
        ...
        obj.nowId = order._id;
        obj.no = order.no;
        Meteor.call('ReplaceBuyOrder.methods.updateShipNumber', obj, (err) => {
            if (!err) {
                message.success('修改成功！');
            } else {
                message.error('修改失败！');
    ...
    _updateStatus(e) {
        ...
        if (obj.status === '已取消') {
            ...
            Meteor.call('ReplaceBuyOrder.methods.cancelOrder', obj, (err) => {
            ...
            Meteor.call('ReplaceBuyOrder.methods.signOrder', obj);
        } else {
            if (!window.confirm(`确认修改为${obj.status}？`)) {
                return;
            }
            Meteor.call('ReplaceBuyOrder.methods.updateStatus', obj, (err) => {
                if (!err) {
    ...
    deleteProduct(orderId, productId) {
        const self = this;
        Meteor.call('ReplaceBuyOrder.methods.deleteProduct', { orderId, productId });
    }
    // 通过条件搜索订单
    _searchBycondition() {
        const self = this;
...
const composer = ({ context }, onData) => {
    ...
    if (!LocalState.get('orderLimiter')) {
        LocalState.set('orderLimiter', {
            selector: {
                replaceBuyerId: Meteor.userId(),
                // replaceBuyerId: 'T2ihiPBuDtwqpLSY5',
                isRemoved: { $ne: true },
                status: {
                    $ne: '已取消',
                },
    ...
    const orderLimiter = LocalState.get('orderLimiter');
    Meteor.subscribe('ReplaceBuyOrder.getCount', orderLimiter.selector);
    const isDataReady = Meteor.subscribe('ReplaceBuyOrder.getData', orderLimiter).ready();
    const count = Counts.get('orderCount');
    const orderList = Collections.ReplaceBuyOrder.find({},{sort: orderLimiter.options.sort}).fetch();
